...
namespace FarmMotion {
    internal sealed class WpfDashboard : Window {
        ...
        FrameworkElement BuildFeedback() {
            ...
            basic.Children.Add(Tuning("sensitivity","Movement sensitivity",.25,3,engine.Settings.Sensitivity,.01,v=>v.ToString("0.00")+"×",v=>engine.UpdateSettings(s=>s.Sensitivity=v),"How strongly vehicle movement becomes feedback."));
            ...
            effects.Children.Add(EffectRow("road","Road tyre buzz",0,.2,engine.Settings.RoadTexture,.01,FeedbackSolo.Road,v=>engine.UpdateSettings(s=>s.RoadTexture=v),"Eligible agricultural tyres rolling on road-class ground.",v=>(v*25).ToString("0.##")+"% of cap"));
            var advancedContent=new StackPanel { Margin=new Thickness(16) }; advancedContent.Children.Add(effects);
            var detail=new StackPanel(); detail.Children.Add(Tuning("texturePitch","Texture frequency",12,45,engine.Settings.TextureFrequency,1,v=>v.ToString("0")+" Hz",v=>engine.UpdateSettings(s=>s.TextureFrequency=v),"Wheel texture carrier rate; controller motor speed is not an exact pitch.")); motionControls.Add(sliders["texturePitch"]);
        ...
        static string SoloName(FeedbackSolo solo) { return solo==FeedbackSolo.Bumps?"Individual bumps":solo==FeedbackSolo.Body?"Body movement":solo==FeedbackSolo.Texture?"Fine texture":solo==FeedbackSolo.Road?"Road tyre buzz":"Original movement"; }
